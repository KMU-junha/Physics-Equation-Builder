...
* **2026 국민대학교 전자공학부 창의설계 경진대회**
    ...
    * <img src="Poster_Physics_Equation_Builder.jpg" width="400">

## 2. Motivation & Problem Definition (배경 및 문제 정의)
* **수치형 데이터 탐색의 한계 (Numerical Data):**
    * 기존 SINDy, GP 기반 기호 회귀 모델 등은 실측 데이터에 포함된 가우시안 노이즈에 취약함. 노이즈가 기저 함수의 계수 추정을 교란하여 허위 항(Spurious term)이 쉽게 활성화됨.
...
* **결과:**
    ...
    * 캐시 활성화($C_{on}$) 조건에서는 50회 전 시행(100%) 수렴에 성공하였으며, 심층 탐색 구간에서 평균 **27%** 의 캐시 재사용률을 보임.
    * 캐시 도입을 통해 최악 소요 시간을 3,044초에서 **59초** 로 억제하여 탐색의 안정성을 개선함.
...
* **도메인 지식을 반영한 탐색 공간의 압축:**
    ...
    * 알고리즘의 맹목적인 탐색을 제한하고 도메인 지식을 규칙화하여 시스템에 주입함으로써, 프로그램의 수행 속도와 수렴 성공률을 끌어올리는 시스템 엔지니어링을 배웠습니다.
